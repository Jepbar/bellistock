...
const (
	...
	sqlUpdateIncomeTransfer                = `update money_transfers set store_id = $1, type_of_account = $2, currency = $3, categorie =$4, customer = $5 ,project = $6, type_of_income_payment =$7, total_payment_amount =$8 , date = $9, user_id = $10, keyword =$11, update_ts = $12`
	...
	if function.IsItAvaiableForDeletingTransfer(date) == true && Editorid == Userid || RoleOfEditor == "Admin" {
		...
		if IsEverythingOk == true {
			...
			if CanIinsertmessage == true {
				k := true
				rows, err := conn.Exec(context.Background(), sqlUpdateIncomeTransfer, intStoreid, typeOfAccount, currency, categorie, customer, project, typeOfIncomePayment, intTotalPaymentAmount, time1, Editorid, keyword, CurrentTime)
				if rows == nil {
					fmt.Println(rows, err)
					k = false
				}
